...
struct Transition {
  ...
  Transition(time_t t, time_t l, int idx)
      : gmttime(t), localtime(l), localtimeIdx(idx) {}
};
...
class File : Uncopyable {
 public:
  File(const char* file) : fd_(::fopen(file, "rb")) {}

  ~File() {
    if (fd_) ::fclose(fd_);
  }

  bool valid() const { return fd_; }

  std::string readBytes(int n) {
    char buf[n];
    ssize_t nr = ::fread(buf, 1, n, fd_);
  ...
  int32_t readInt32() {
    int32_t x = 0;
    ssize_t nr = ::fread(&x, 1, sizeof(int32_t), fd_);
    if (nr != sizeof(int32_t)) throw std::logic_error("bad int32 data");
    return be32toh(x);
  }

  uint8_t readUInt8() {
    uint8_t x = 0;
    ssize_t nr = ::fread(&x, 1, sizeof(uint8_t), fd_);
    if (nr != sizeof(uint8_t)) throw std::logic_error("bad uint8 data");
    return x;
  }
...
TimeZone::TimeZone(const char* zonefile) : data_(new TimeZone::Data) {
  if (!readTimeZoneFile(zonefile, data_.get())) data_.reset();
}
